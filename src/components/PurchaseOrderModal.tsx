...
import type { Supplier } from '../data/suppliers'; // Import global Supplier type
import type { PurchaseOrderItem } from '../data/purchaseOrders'; // Import global PurchaseOrderItem type
...
export default function PurchaseOrderModal({ isOpen, onClose, onSave }: PurchaseOrderModalProps) {
  ...
  const [filteredProducts, setFilteredProducts] = useState<Product[]>([]); // Products for the selected supplier
  ...
  useEffect(() => {
    if (selectedSupplier) {
      ...
      if (selectedSupplierName) {
        setFilteredProducts(allProducts.filter(p => p.supplier === selectedSupplierName));
      } else {
        setFilteredProducts([]);
      }
    } else {
    ...
    // Clear order items when supplier changes
    setOrderItems([]);
  }, [selectedSupplier, allProducts, suppliers]);

  const handleAddItem = () => {
    if (!selectedSupplier || filteredProducts.length === 0) {
      setError("Por favor, seleccione un proveedor y asegúrese de que tenga productos disponibles antes de agregar ítems.");
      return;
    }
    const defaultProduct = filteredProducts[0]; // Use filteredProducts
    setOrderItems([
      ...orderItems,
  ...
  const handleItemChange = (index: number, field: 'product_id' | 'quantity', value: string | number) => {
    ...
    if (field === 'product_id') {
      const product = filteredProducts.find(p => p.id === value); // Use filteredProducts
      ...
      <div className="po-modal-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="po-item-list-header">
            ...
            <button type="button" onClick={handleAddItem} className="po-add-item-btn" disabled={!selectedSupplier || filteredProducts.length === 0}>+ Agregar Item</button>
          </div>
